...
const fetchCategoria = async () => {
    ...
    try {
        ...
        categoria.value.nombre = data.nombre;
        categoria.value.estado = data.estado;
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
...
<template>
    <Dialog v-model:visible="dialogVisible" header="Editar Categoría" modal :closable="true" :closeOnEscape="true"
        ...
        <template #footer>
            ...
            <Button label="Guardar" icon="pi pi-check" severity="contrast" @click="updateCategoria" :loading="loading" />
        </template>
    </Dialog>
</template>
